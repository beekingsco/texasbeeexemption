      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {['all', 'new', 'contacted', 'client', 'closed'].map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                style={{
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 60, background: C.white, borderRadius: 16, border: '1px solid #e2e8f0' }}>
            ...
            <p style={{ color: C.gray, fontSize: 14 }}>Leads will appear here as visitors use your branded link.</p>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ background: '#F8FAFC', borderBottom: '2px solid #e2e8f0' }}>
                  {['Date', 'Owner', 'Email', 'Property Address', 'County', 'Acres', 'Est. Savings', 'Status'].map(h => (
                ...
                {filtered.map(lead => (
                  <>
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 14px', color: C.blue }}>—</td>
                      <td style={{ padding: '12px 14px', color: C.gray, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{lead.propertyAddress}</td>
